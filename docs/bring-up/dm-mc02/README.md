...
完整导出在 `TEST_COMPLETE` 后只保留约 3.002 秒的浏览器窗口，未达到规定的
5 秒，因此 5 秒保持失能的浏览器观察验收尚不完整；后续需由控制器进行实时
`STATE=DISABLED`/CAN 健康采样。`dropped_logs=240` 在运动前已存在、整段导出
中保持恒定，符合无监听器时 CDC 严格丢弃日志的行为，不作为本次运动故障报告。
